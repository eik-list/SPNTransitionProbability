...
static const size_t PRECISION = 300;
static NTL::RR Q32;
 ...
 * @return
 */
static int getNumActiveBytesInColumn(uint8_t state[4][4], int k) {
    int w = 0;
 ...
 * @return
 */
static int getNumActiveBytesInRow(uint8_t state[4][4], int k) {
    int w = 0;
 ...
 */
static void
getColumnAndRowWeightPattern(int wpattern0[8], int wpattern1[8], const int p1) {
    uint8_t S1[4][4];
    ...
    for (int i = 0; i < 4; i++) {  // getNumActiveBytesInColumn of i-th column
        ...
        wpattern0[i + 4] = getNumActiveBytesInRow(S1, i);
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        wpattern1[i + 4] = getNumActiveBytesInRow(S1, i);
    }
}
 ...
 * @return
 */
static int computeIndex(const int pattern[8]) {
    int result = 0;
...
AESRowAndColumnTransitionAlgorithm::AESRowAndColumnTransitionAlgorithm() {
    initializeConstants();
}

// ------------------------------------------------------------------------

void AESRowAndColumnTransitionAlgorithm::buildFirstByteDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
    distribution[0].SetOutputPrecision(PRECISION);
    distribution[1] = 1;
}

// ------------------------------------------------------------------------

void AESRowAndColumnTransitionAlgorithm::buildFirstDiagonalDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
...
void AESRowAndColumnTransitionAlgorithm::precomputeShiftRowsMatrix() {
    ...
    for (int bytePatternAsInt = 0;
        ...
        int bytePatternBeforeShiftRows[8];
        int bytePatternAfterShiftRows[8];

        // ---------------------------------------------------------------------
        // This function is the main one here.
        // Given bytePatternAsInt = (...., ...., ...., ....)
        // ---------------------------------------------------------------------

        getColumnAndRowWeightPattern(bytePatternBeforeShiftRows,
                                     bytePatternAfterShiftRows,
                                     bytePatternAsInt);

        const int I = computeIndex(bytePatternBeforeShiftRows);
        const int J = computeIndex(bytePatternAfterShiftRows);

        NTL::RR M;
        M = 1;

        for (int i = 0; i < 4; i++) {
            M = M * BINOMIAL_COEFFICIENTS[bytePatternBeforeShiftRows[i]];
        }
...
void AESRowAndColumnTransitionAlgorithm::precomputeMixColumnsMatrix() {
    // ------------------------------------------------------------------------
...
void AESRowAndColumnTransitionAlgorithm::readZTable(const std::string &path) {
    NTL::ZZ Z_ZZ[16][16];
...
bool AESRowAndColumnTransitionAlgorithm::haveMatricesBeenPrecomputed() const {
...
void AESRowAndColumnTransitionAlgorithm::createMatrix(NTL::mat_RR &matrix,
...
const {
    ...
    matrix[0][0].SetOutputPrecision(PRECISION);
...
void AESRowAndColumnTransitionAlgorithm::writeMatrix(const std::string &path,
...
void AESRowAndColumnTransitionAlgorithm::computeOutputProbabilityForAES(
    NTL::RR &probability,
    ...
    outputDistribution = inputDistribution * TMC;
    ...
    for (int column0 = 0; column0 < 5; column0++) {
        for (int column1 = 0; column1 < 5; column1++) {
            for (int column2 = 0; column2 < 5; column2++) {
                for (int column3 = 0; column3 < 5; column3++) {
                    if (outputInterests[column0][column1][column2][column3]) {
                        const int indices[4] = {column0, column1, column2,
                                                column3};
...
void AESRowAndColumnTransitionAlgorithm::computeOutputProbabilityForPRP(
    NTL::RR &probability,
    const bool outputInterests[5][5][5][5][5][5][5][5]) const {
    probability.SetPrecision(PRECISION);
