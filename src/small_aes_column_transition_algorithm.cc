...
SmallAESColumnTransitionAlgorithm::SmallAESColumnTransitionAlgorithm() {
    initializeConstants();
}

// ------------------------------------------------------------------------

void SmallAESColumnTransitionAlgorithm::buildSingleElementDistribution(
...
void SmallAESColumnTransitionAlgorithm::buildUniformDifferencesDistribution(
    NTL::vec_RR &distribution,
    const uint8_t columnPattern[4]) const {
...
void SmallAESColumnTransitionAlgorithm::buildFirstByteDistribution(
...
void SmallAESColumnTransitionAlgorithm::buildFirstDiagonalDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
    ...
    for (int i = 1; i < 5; i++) {
        distribution[i] =
            BINOMIAL_COEFFICIENTS[i] * POWERS_OF_15[i] / (Q16 - 1);
    }
}

// ------------------------------------------------------------------------

void SmallAESColumnTransitionAlgorithm::buildFirstTwoDiagonalsDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
    ...
    for (int numActiveBytesInColumn0 = 0;
        ...
        for (int numActiveBytesInColumn1 = 0;
            ...
            distribution[index] =
                ...
                POWERS_OF_15[numActiveBytesInColumn0] *
                BINOMIAL_COEFFICIENTS[numActiveBytesInColumn1] *
                POWERS_OF_15[numActiveBytesInColumn1] /
                (Q32 - 1);
        }
    }
}
...
void SmallAESColumnTransitionAlgorithm::buildFirstThreeDiagonalsDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
        ...
        for (int numActiveBytesInColumn1 = 0;
            ...
            for (int numActiveBytesInColumn2 = 0;
                ...
                distribution[index] =
                    ...
                    POWERS_OF_15[numActiveBytesInColumn2] /
                    (Q48 - 1);
            }
        }
    }
...
void SmallAESColumnTransitionAlgorithm::buildAllDiagonalsDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
            ...
            for (int numActiveBytesInColumn2 = 0;
                ...
                for (int numActiveBytesInColumn3 = 0;
                    ...
                    distribution[index] =
                        ...
                        POWERS_OF_15[numActiveBytesInColumn3] /
                        (Q64 - 1);
                }
            }
        }
...
void SmallAESColumnTransitionAlgorithm::buildFirstNDiagonalsDistribution(
    NTL::vec_RR &distribution,
...
void SmallAESColumnTransitionAlgorithm::buildFirstTwoBytesDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
...
void SmallAESColumnTransitionAlgorithm::buildFirstThreeBytesDistribution(
    NTL::vec_RR &distribution) const {
    distribution.SetLength(625);
    distribution[0].SetPrecision(PRECISION);
...
void SmallAESColumnTransitionAlgorithm::buildFirstNBytesDistribution(
    NTL::vec_RR &distribution,
...
void SmallAESColumnTransitionAlgorithm::precomputeShiftRowsMatrix() {
    // ------------------------------------------------------------------------
...
void SmallAESColumnTransitionAlgorithm::precomputeMixColumnsMatrix() {
    // ------------------------------------------------------------------------
...
void SmallAESColumnTransitionAlgorithm::readZTable(const std::string &path) {
    NTL::ZZ Z_ZZ[16][16];
...
bool SmallAESColumnTransitionAlgorithm::haveMatricesBeenPrecomputed() const {
...
void SmallAESColumnTransitionAlgorithm::createMatrix(NTL::mat_RR &matrix,
...
void SmallAESColumnTransitionAlgorithm::writeMatrix(const std::string &path,
...
void SmallAESColumnTransitionAlgorithm::computeOutputProbabilityForAES(
    NTL::RR &probability,
...
void SmallAESColumnTransitionAlgorithm::computeOutputProbabilityForPRP(
    NTL::RR &probability,
        ...
        for (int v1 = 0; v1 < 5; v1++) {
            for (int v2 = 0; v2 < 5; v2++) {
                for (int v3 = 0; v3 < 5; v3++) {
                    if (outputInterests[v0][v1][v2][v3]) {
                        probability +=
                            BINOMIAL_COEFFICIENTS[v0] * POWERS_OF_15[v0]
                            * BINOMIAL_COEFFICIENTS[v1] * POWERS_OF_15[v1]
                            * BINOMIAL_COEFFICIENTS[v2] * POWERS_OF_15[v2]
                            * BINOMIAL_COEFFICIENTS[v3] * POWERS_OF_15[v3]
                            / (Q16 * Q16 * Q16 * Q16);
                    }
                }
            }
